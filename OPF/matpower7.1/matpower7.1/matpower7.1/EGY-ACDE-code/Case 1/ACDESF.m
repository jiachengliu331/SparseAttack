...
Max_FES = 30000;
Max_Gen = 600; maxrun = 1; gn = 4; % gn是约束数量,Max_Gen = 600运行代数
...
SE = [];%进化成功
 ...
 for runs=1:maxrun 
   ...
   %g是约束，Q发电机的无功功率，v负载总线电压，s线路的功率
   for i=1:NP
      [val(i,1),g(i,:)] = Func(pop(i,:));%val适应度
   end
...
for gen=1:Max_Gen
    ...
    % CR排序机制
    [CR,~] = sort(CR);%CR排序
    [~,indices] = sort(val);%适应度排序，返回indices是val里的行数位置

     %%在种群前p%中随机选择一个个体作为pbest
     pid = ceil(NP*p);%ceil是四舍五入
     pbestid = randi(pid);%生成1—pid的随机数
     Pbest = pop(indices(pbestid),:);
     
     for i =1:NP
        %%变异
        [Xr1,Xr2] = selectR(pop,i);%随机选取两个不相同的个体，且与目标个体不同 
        [m,~] = size(SE);
        %不采用进化成功的方向
        if rand<=0.5 || m<1
    ...
    nfeval=nfeval+NP;
    %将在目标X与V中进行选择
    A1 = vertcat(pop,newpop);%,newpop2);vertcat垂直连接数组
