...
%--------------------------------------------------------------------------
    ...
    methods(Access = ?GUI)
        ...
        function obj = module_app(GUI)
            ...
            obj.app.buttonB(2)  = GUI.APP(1,3,uibutton(tempGrid,'Text','Del','Enable',false,'FontSize',10,'Tooltip','Delete the dataset','BackgroundColor','w','ButtonpushedFcn',{@obj.cb_updateProblem,-1}));
    ...
    methods(Access = private)
        ...
        function PRO = cb_validation(obj,para,paraname)
            ...
            try
                if length(Str) > 6
                    PRO = UserProblem('N',para{1},paraname,para{2},'encoding',Str{1},'lower',Str{2},'upper',Str{3},'parameter',Str{4},'initFcn',Str{5},'decFcn',Str{6},'objFcn',Str{7},'conFcn',Str{8});
                else
                    PRO = UserProblem('N',para{1},paraname,para{2},'encoding',Str{1},'lower',Str{2},'upper',Str{3},'parameter',Str{4},'initFcn',Str{5},'evalFcn',Str{6});
                end
                PRO.Initialization();
            catch err
        ...
        function cb_loadProblem(obj,~,~)
            ...
            if ischar(file)
                try
                    ...
                    Str{4} = regexp(fgetl(f),'(?<=''parameter'','').*(?='',...)','match','once');
...
function Str = GetSetProblem(obj,Str)
    ...
    if nargin > 1	% Set the problem
        ...
        obj.app.editB(4).Value  = Str{4};               % Dataset
